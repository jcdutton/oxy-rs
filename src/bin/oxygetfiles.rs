...
async fn run_device_loop(peripheral: &Peripheral, state: &mut AppState) -> Result<(), Box<dyn Error>> {
    ...
    //let ble_uuid = "14839ac4-7d7e-415c-9a42-167340cf2339";
    ...
    // Send request
    let result1 = get_info(peripheral, write_char, &mut notification_stream, &mut buf1).await?;

    let mut json3: String  = "".to_string();
    let result1 = get_info_buf_to_json(&mut buf1, &mut json3);

    let result2 = get_info_json_to_files(&mut json3, &mut filenames1);
    println!("FileList vector2: {:?}", filenames1);

    for filename0 in filenames1 {
        // .create_new(true) ensures it ONLY creates the file if it's missing.
        // If the file exists, this will return an Error.
        let filename1: String = filename0.to_string();
        let filename2 = filename1.clone() + ".dat";
        ...
        match file_result {
            Ok(mut file) => {
                let result3 = get_file(state, peripheral, write_char, &mut notification_stream, &filename1, &mut file_contents1).await?;
                file.write_all(&file_contents1)?;
